...
def crop_center(img, crop_width, crop_height):
    """
    Crops the center of the image to the specified width and height.
    """
        ...
        while True:
            with shared_data['lock']:
                if shared_data['frame'] is not None:
                    ...
                    shared_data['frame'] = None  # Reset after grabbing
                else:
                    frame = None

            if frame is None:
                # No frame available
                time.sleep(0.001)  # Sleep briefly to prevent CPU hogging
                continue
            ...
            min_contour_area = 20  # Minimum area to be considered a blob
            filtered_contours = []
            for cnt in contours:
